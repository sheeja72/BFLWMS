...
public class AuditSaveChangesInterceptor(IServiceScopeFactory scopeFactory) : SaveChangesInterceptor
{
    ...
    private void AppendAuditRows(DbContext? ctx)
    {
        ...
        var logs = new List<AiwmsAuditLog>();

        foreach (var entry in ctx.ChangeTracker.Entries().ToList())
        {
            if (entry.Entity is AiwmsAuditLog) continue;
        ...
        if (logs.Count > 0) ctx.Set<AiwmsAuditLog>().AddRange(logs);
    }
...
public class ActionLogger(AiwmsDbContext db, ICurrentUser currentUser) : IActionLogger
{
    ...
    {
        db.AuditLogs.Add(new AiwmsAuditLog
        {
            EntityName  = entityName,
            EntityKey   = entityKey,
            Action      = action,
